...
function visitNeighbors(row: number, col: number, distFromStart: number, path: string[], lastHeight: string) {
    let newHeight = grid.getCell(row, col);
    if (newHeight === 'E') newHeight = 'z'
    const newPath = [...path, `${row},${col}`];

    if (lastHeight === 'b' && newHeight === 'a') {
    ...
    if (newHeight.ascii() < lastHeight.ascii() - 1) {
        return;
    }
...
visitNeighbors(startingCell.row, startingCell.col, 0, [], 'z');
